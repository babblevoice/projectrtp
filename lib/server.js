...
class channel {
  ...
  */
  async mix( other ) {
    ...
    else
    {
      ...
      for ( let bridge in other.bridges )
      {
        // Look for an existing connection and mix with the local ( on the same node ) channel ( bridge )
        if ( bridge.remotenode.status.instance === this.remotenode.status.instance )
        {
          combinemixes( this.mixes, bridge.mixes )
          this._write( {
            "channel": "mix",
            "other": {
              "id": bridge.id,
      ...
      bridge1.remote({ "address": bridge2.local.adress, "port": bridge2.local.port, "codec": this.codec })
      bridge2.remote({ "address": bridge1.local.adress, "port": bridge1.local.port, "codec": other.codec })
      ...
      combinemixes( this.mixes, bridge1.mixes )
      combinemixes( other.mixes, bridge2.mixes )

      // Keep track of bridges on each node
      this.bridges.push( bridge1 )
      this.bridges.push( bridge2 )
      other.bridges.push( bridge1 )
      other.bridges.push( bridge2 )
  ...
  */
  unmix(){
    const index = this.mixes.indexOf( this )
    if( index != -1 )
    {
    ...
    if( "open" === msg.action ) {
      ...
      if ( this.bridges.length != 0 )
      {
        if ( this.mixes.length - ( this.bridges.length / 2 )  > 0 )
        {
          ...
          this.mixes[0].bridges = this.bridges
        }
        else
        {
...
function combinemixes( mix1, mix2 ) {
  // This doesn't work ( does not modify mix1/mix2 outside of scope of the function )
  mix1 = mix2 = [...new Set([...mix1,...mix2])]
}
