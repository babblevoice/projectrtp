...
class channel {
  ...
  async openchannel( options, callback ) {
    ...
    if ( this.connectednode ) {
      ...
      newchannel.connectednode = this.connectednode
      newchannel.channels.push( newchannel )
      newchannel._write( request )
      return newchannel
    } else if ( this.remotenode ) return _createforlisten( request, this.remotenode, undefined, this.openresolve, this.openreject)
      else return await _createforconnect( request, undefined, callback )
  }
  ...
  */
  async mix( other ) {
    ...
    // Only if there is a remotenode, should also add support for connectednodes
    if ( this.remotenode ) {
  ...
  */
  ...
  _on( msg ) {
    ...
    if( "open" === msg.action ) {
      ...
      if( this.connection && !this.channels ) {
  ...
  */
  ...
  openchannel( options, callback ) {
    return new Promise( async ( resolve, reject ) => {
      ...
      let r
      if ( nodes.size > 0 ) r = channel._createforlisten( request, node, callback, resolve, reject )
      else r = await channel._createforconnect( request, callback, resolve, reject )
    } )
  }
