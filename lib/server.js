...
class channel {
   ...
   */
  static _createforconnect( request, remotenode, callback ) {

    let newchannel = new channel()
    if( callback ) newchannel.em.on( "all", callback )

    newchannel.connectednode = remotenode

    return new Promise( resolve => {

      newchannel._onsocketreadypromiseresolve = resolve
      newchannel.connection = { "connections": [ newchannel ] }
      newchannel.connection.sock = net.createConnection( remotenode.port, remotenode.host )
      ...
      newchannel.connection.sock.on( "data", ( data ) => {
        message.parsemessage( state, data, ( receivedmsg ) => {
          /* find channel */
          for( chan in newchannel.connection.connections ) {
            if( chan.id == receivedmsg.id ) {
              chan._on( receivedmsg )

              if ( resolve ) resolve( newchannel )
              resolve = false
              break
            }
          }
          ...
          /* global emitter */
          if( newchannel.em ) newchannel.em.emit( "projectrtp.msg", receivedmsg )
          
        } )
      } )
      newchannel.connection.sock.on( "error", () => {
  ...
  async openchannel( options, callback ) {
    ...
    if( this.remotenode ) {
      this.remotenode
    } else if ( this.connectednode ) {
      let newchannel = new channel()
      newchannel.connection = this.connection
      this.connection.connections.push( newchannel )
    }

    if( callback ) newchannel.em.on( "all", callback )

    options.nodeinstance = this.remotenode.status.instance

    let newchannel = new channel( /* figure this out! */ )
    this.connectioninfo.channels++
    newchannel.connectioninfo = this.connectioninfo
    newchannel.em = this.em
    //newchannel.em.on( "all",  )
  ...
  */
  close() {
    if ( this.connectioninfo !== undefined ) {
      this.connectioninfo.channels -= 1
    }

    if ( this.connectioninfo <= 0 ) {
  ...
  */
  async mix( other ) {
    ...
    else
    {
      ...
      // Use private adress if it exists
      let adress1 = bridge1.local.privateaddress
      if ( adress1 === undefined )
      {
        adress1 = bridge1.local.address
      }
      let adress2 = bridge2.local.privateaddress
      if ( adress2 === undefined )
      {
        adress2 = bridge1.local.address
      }
      // Point bridges to each other
      bridge1.remote({ "address": adress2, "port": bridge2.local.port, "codec": this.codec })
      bridge2.remote({ "address": adress1, "port": bridge1.local.port, "codec": other.codec })
  ...
  */
  openchannel( options, callback ) {
    return new Promise( ( resolve, reject ) => {
      ...
      channels.set( r.id, r )
      if ( !options.connectioninfo && listeningnodes.length && !nodes.size ) {
        // Add host here ( from sock? ) so it is on the correct node, so why do we need a list of hosts? ( the function below )
        r.connectioninfo = { "channels": 1, "connection": this.connect( 9002, node.sock ) }
      } else {
