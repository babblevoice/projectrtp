...
class channel {
  ...
  */
  ...
  async openchannel( options, callback )
  {
    ...
    options.connectioninfo = this.connectioninfo
...
class rtpserver {
  ...
  */
  listen() {
    ...
    /**
  @summary Connect to a node on after openchannel.
  @param {number} port
  @param {string} host
  @return {Promise<rtpnode>} - Promise which resolves to a server
  */
...
/**
 * We either call listen or addnode. When listen is called, nodes should call connect to connect back to us
 * to establish 1 connection for a control connection.
 * @param { number } port - the port to listen for incoming connections for control connections from nodes
 * @param { string } address - ip to listen to (or '::' for all)
 * @param { object } em - event emitter to pass events back to
 */
