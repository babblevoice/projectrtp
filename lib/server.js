...
*/
  ...
  /*unmix() {
    channel, index = findtheotherforthischannel()
    if( 0 === bridges[ index ].other.channels.length ) {
      bridges[ index ].left.unmix()
      bridges[ index ].left.close()
      bridges[ index ].right.unmix()
      bridges[ index ].right.close()
      removefrombridgesarray()
    }
  }*/
  ...
  */
  async mix( other ) {
    ...
    else
    {
      ...
      {
        // Look for an existing connection and mix with the local ( on the same node ) channel ( bridge )
      ...
      // Keep track of bridges on each node.
      // Check for existing nodebridges[] and decide on main
      if ( !this.addnodebridge( other, false ) )
      {
...
addnodebridge( other, local )
{
  if ( this.nodebridges.length !=0 && other.nodebridges != 0 )
  {
    if ( !local )
    {
      this.nodebridges.push({
    ...
    for ( let nodebridge in other.nodebridges )
    {
      ...
      nodebridge.main.bridge = this.nodebridges[0].bridge
      this.nodebridges.push( nodebridge )
    }
    for ( chnl in other.mixes )
    {
...
/*unmix() {
  ...
  */
  unmix(){
    for ( let nodebridge in this.nodebridges )
    {
      if ( nodebridge.other.node === this.remotenode.status.instance )
      {
        const index = nodebridge.other.channels( this );
        if ( index > -1 )
        {
  ...
  */
  ...
  _on( msg ) {
    if( "open" === msg.action ) {
      ...
      //node examples/simplenode.js
      // Was *this* a main channel?
      if ( this.bridges.length != 0 )
      {
        console.log( this.bridges.length, this.mixes.length )
        if ( this.mixes.length - ( this.bridges.length / 2 )  > 0 )
        {
          // Transfer ownership if any channel is left on a node
          for ( let chnl in this.mixes )
          {
            if( !this.bridges.includes( chnl ) && !typeof chnl === 'string')
            {
              chnl.bridges = this.bridges
            }
          }
        }
        else
        {
          // Otherwise close all bridges
          for ( let bridge in this.bridgess )
          {
            if ( bridge )
            {
              bridge.close()
...
function combinemixes( mix1, mix2 ) {
  for( let chnl in mix2)
  {
    mix1.push( chnl )
    chnl.mixes = mix1
  }
  for ( let chnl in mix1 )
  {
    chnl.mixes = mix1
  }
}
