  ...
  */
  async mix( other ) {
    ...
    else
    {
      ...
      if( !bridge_found )
      {
        bridge1 = await s.openchannel({}, false, this.remotenode)
        bridge2 = await s.openchannel({}, false, other.remotenode)
  ...
  */
  openchannel( options, callback, selected_node ) {
    return new Promise( ( resolve, reject ) => {
      ...
      {
      if ( undefined === related || undefined !== related) {
        node = randomenode()
      } else {
  ...
  _onnewconnection( sock ) {
    ...
    sock.on( "data", ( data ) => {
      message.parsemessage( state, data, ( receivedmsg ) => {
        ...
        console.log(receivedmsg.status)
