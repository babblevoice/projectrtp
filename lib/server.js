...
class channel {
  ...
  */
  async mix( other ) {
    if( other.remotenode === this.remotenode )
    {
      this._write( {
        "channel": "mix",
        "other": {
    ...
    else
    {
      var bridge1
      var bridge2
      let bridge_found = false
      for( let node_id of bridgechannels.keys() ) 
      {
        if ( !bridge_found )
        {
          if( node_id === this.remotenode.instance )
          {
            bridge1 = bridgechannels.get( node_id ).channel1
            bridge2 = bridgechannels.get( node_id ).channel2
            bridge_found = true
          }
          else if( node_id === other.remotenode.instance )
          {
            bridge1 = bridgechannels.get( node_id ).channel2
            bridge2 = bridgechannels.get( node_id ).channel1
            bridge_found = true
          }
        }
      }
      ...
      if( !bridge_found )
      {
        bridge1 = await s.openchannel( { "selected_node": this.remotenode.status.instance }, false )
        bridge2 = await s.openchannel( {"selected_node": other.remotenode.status.instance }, false )

        bridge1.remote({ "address": bridge2.remoteadress, "port": bridge2.remoteport, "codec": this.codec })
        bridge2.remote({ "address": bridge1.remoteadress, "port": bridge1.remoteport, "codec": other.codec })
        ...
        bridgechannels.set( this.remotenode.instance, bridgechannel )
      }
...
class rtpserver {
  ...
  */
  ...
  connected.
  */
  openchannel( options, callback, selected_node ) {
    return new Promise( ( resolve, reject ) => {
      ...
      if (options.selected_node === undefined)
      {
      ...
      {
        node = nodes.get( options.selected_node )
        // check later how many are open with what instances
        console.log("Trying to get: ", options.selected_node)
        console.log("Available nodes:")
        for( let node of nodes.keys() ) 
        {
          console.log(nodes.get(node).status)
        }
      }
