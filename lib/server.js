  ...
  */
  async mix( other ) {
    if( other.remotenode.status.instance === this.remotenode.status.instance )
    {
      ...
      this.mixes.push( other )
      other.mixes.push( this )
    }
    else
    {
      var connectionfound = false
      for ( let node in other.nodes )
      {
        if ( node.channel.remotenode.status.instance === this.remotenode.status.instance )
        {
          connectionfound = true
          node.channel.mixes.push( this )
          this.mixes.push( node.channel )
          this._write( {
            "channel": "mix",
            "other": {
              "id": node.channel.id,
      ...
      channels.set( bridge1.uuid, bridge1 )
      channels.set( bridge2.uuid, bridge2 )
  ...
  */
  ...
  _on( msg ) {
    if( "open" === msg.action ) {
      ...
      while ( this.mixes.length > 0 )
      {
        const index = this.mixes.indexOf( this.mixes[0] )
        if ( channels.has( this.mixes[0].id ) )
        {
          this.mixes[0].nodes = this.nodes
          this.mixes[0].mixes = this.mixes
          this.mixes.splice(index, 1)
      ...
      if ( !ownershiptransferred )
      {
        ...
        {
          if( node.bridge1 !== undefined )
          {
            if ( channels.has( node.bridge1.id ) )
            {
              channels.delete( node.bridge1.id )
            }
          }
          if( node.bridge2 !== undefined )
          {
            if ( channels.has( node.bridge2.id ) )
            {
              channels.delete( node.bridge2.id )
            }
          }
