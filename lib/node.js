...
class rtpnode {
  ...
  */
  connect( port, host ) {
    ...
    return new Promise( resolve => {
      ...
      this.connection.on( "data", this._onsocketdataconnect.bind( this ) )
  ...
  listen() {
    ...
    this.server = net.createServer( ( connection ) => {
      ...
      connection.on( "data", this._onsocketdatalisten.bind( this, connection ) )
    } )
  ...
  */
  ...
  destroy() {
    ...
    for( const con of connections ) con.destroy()
   ...
   */
  async _processmessage( msg, connection ) {
    return ( await this._openchannel( msg, connection ) || this._updatechannel( msg, connection ) )
  }
   ...
   * @returns { void }
   */
  _onsocketdatalisten( connection, data ) {
    message.parsemessage( this.messagestate, data, ( msg ) => {
      try {
        this._pre( msg, ( modifiedmsg ) => {
          this._processmessage( modifiedmsg, connection )
        } )
      } catch( e ) {
   ...
   * @returns { void }
   */
  _onsocketdataconnect( data ) {
    message.parsemessage( this.messagestate, data, ( msg ) => {
      try {
        this._pre( msg, ( modifiedmsg ) => {
          this._processmessage( modifiedmsg, this.connection )
        } )
      } catch( e ) {
        console.error( "Unhandled exception in babble-rtp", e )
      }
  ...
  _updatechannel( msg, connection ) {
    ...
    if( msg.channel in channelmap ) {
      ...
      this.send( { ...{ "error": "Unknown method" }, ...channelidentifiers }, connection )
    }
