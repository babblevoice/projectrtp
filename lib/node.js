...
const channelmap = {
  "close": ( chan ) => {
    ...
    if( chan.readstream ) readstream.readstreamend( chan )
    chan.close() 
  },
  ...
  "record": ( chan, msg ) => {
    if( 0 === msg.options.file.indexOf( "/readstream/" ) ) readstream.readstreamstart( chan, msg )
    else chan.record( msg.options )
    },
