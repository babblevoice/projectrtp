...
| `projectrtpcodecx.cpp/.h` | `codec.rs` + `ilbc.rs` | partial | G.711 is pure Rust (spandsp tables). G.722 uses the `ezk-g722` crate. iLBC is a thin safe FFI over the system `libilbc.so` (`rust/src/ilbc.rs`); `Transcoder` recognises up to two iLBC dynamic PTs (self + peer) so `remote.ilbcpt` dynamic-PT negotiation works across a mix. L16 still TODO. Alpine Docker must `apk add ilbc-dev` (build) / `ilbc` (runtime). |
